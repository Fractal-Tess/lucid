<script lang="ts" module>
    ...
    export const motionPopVariants = tv({
        base: "motion-preset-pop",
        variants: {
            duration: {
    ...
    export type MotionPopDuration = VariantProps<
        typeof motionPopVariants
    >["duration"];
    export type MotionPopDelay = VariantProps<
        typeof motionPopVariants
    >["delay"];

    export type MotionPopProps = WithElementRef<
        HTMLAttributes<HTMLDivElement>
    > & {
        duration?: MotionPopDuration;
        delay?: MotionPopDelay;
    };
</script>
...
<script lang="ts">
    let {
        ...
        delay = 0,
        ...restProps
    }: MotionPopProps = $props();
</script>

<div
    bind:this={ref}
    class={cn(motionPopVariants({ duration, delay }), className)}
    {...restProps}
>
    {@render children?.()}
</div>
