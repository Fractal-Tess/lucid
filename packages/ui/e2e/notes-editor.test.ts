...
test.describe("NotesEditor", () => {
  ...
  test("allows editing key points", async ({ page }) => {
    const editButton = page.locator("[data-testid='edit-keypoint-button']").first();
  ...
  test("allows deleting key points", async ({ page }) => {
    const initialCount = await page.locator("[data-testid='delete-keypoint-button']").count();

    const deleteButton = page.locator("[data-testid='delete-keypoint-button']").first();
    await deleteButton.click();

    const newCount = await page.locator("[data-testid='delete-keypoint-button']").count();
  ...
  test("switches between edit and preview tabs", async ({ page }) => {
    ...
    await expect(page.locator("[data-testid='notes-content-input']")).not.toBeVisible();
  });
